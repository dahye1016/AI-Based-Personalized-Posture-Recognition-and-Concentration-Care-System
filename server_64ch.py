...
#    seat_1~32는 8행(앞->뒤) x 4열(좌->우) 블록이 순서대로 나열된 것이므로
#    (extract_64ch_dataset.py 기준), 이를 다시 4분면으로 묶어서 요약함.
#    - 방석: 앞/뒤 x 좌/우 4개
#    - 등받이: 위/아래 x 좌/우 4개
#    CNN이 특정 자세를 헷갈려할 때, 이 8개 요약값을 보면 "등받이 압력이
#    거의 0이라 등받이를 안 쓰는 자세들끼리 구분이 어려웠구나" 같은 걸
#    사람이 바로 눈으로 확인할 수 있음.
# =====================================================================
def summarize_regions(seat_values, back_values):
    seat = np.array(seat_values, dtype=np.float32).reshape(8, 4)   # (row, col)
    back = np.array(back_values, dtype=np.float32).reshape(8, 4)

    def quadrant_means(grid):
        top, bottom = grid[:4], grid[4:]
        return {
            "top_left": float(top[:, :2].mean()),
            "top_right": float(top[:, 2:].mean()),
            "bottom_left": float(bottom[:, :2].mean()),
            "bottom_right": float(bottom[:, 2:].mean()),
        }

    seat_q = quadrant_means(seat)
    back_q = quadrant_means(back)
    return {
        "seat_front_left": seat_q["top_left"],
        "seat_front_right": seat_q["top_right"],
        "seat_rear_left": seat_q["bottom_left"],
        "seat_rear_right": seat_q["bottom_right"],
        "back_upper_left": back_q["top_left"],
        "back_upper_right": back_q["top_right"],
        "back_lower_left": back_q["bottom_left"],
        "back_lower_right": back_q["bottom_right"],
    }
