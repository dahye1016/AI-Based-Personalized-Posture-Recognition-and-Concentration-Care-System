...
/// 실채널 31개(ch0~ch30)의 raw 압력값을 앞/뒤/좌/우 구역 평균으로 묶어
/// 조건 판정한다. 구역 정의는 [PostureLayout] 이 단일 소스다.
///
/// 구역이 가리키는 부위 (MDXS-16-5610, 6/15/10 배치):
///   · front = 허벅지(ch21~30) — 앞으로 숙이면 커진다
///   · back  = 꼬리뼈(ch0~5)   — 골반을 뒤로 굴려 기대면 커진다
///   · 가운데 엉덩이뼈 행(ch6~20)은 앞뒤 판정에 쓰지 않는다.
///     어느 자세에서도 하중이 실려 앞뒤를 가르지 못하기 때문이다.
///     좌우 비교에는 들어간다 (중심선 ch13 제외).
///
/// ⚠️ 임계값(threshold)은 6채널 기준을 옮겨온 초기값이라,
///    실제 센서 raw 값 범위(예: 0~4095)에 맞춰 실물 연결 때
///    보정이 필요하다. (아래 상수만 조정하면 됨)
///    특히 front/back 은 구역 크기가 10칸 / 6칸으로 달라졌으므로
///    _frontStrong 과 _backStrong 을 같은 값으로 두는 근거가 없다.
